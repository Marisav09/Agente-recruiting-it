...
def ejecutar_test():
    ...
    try:
        print("🤖 Pasando datos por el Árbol de Decisión de Machine Learning...")
        df_enriquecido = enriquecer_dataset()
        print("✅ ¡Dataset completado por la IA y guardado con éxito!")
        print("-" * 50)
    ...
    print(f"📊 Dataset cargado con éxito: {metrics['filas']} registros detectados.")
    ...
    print(f"🕵️ Agente evaluando candidatos para: '{tecnologia_buscada}'")
    print(f"💵 Presupuesto Máx: ${sueldo_max} | Exp. Mínima: {exp_min} años\n")
    ...
    for i, candidato in enumerate(top_3, 1):
        ...
        print(f"    💼 Exp: {exp_cand} años | Sueldo: {sueldo_texto}")
        print(f"    🤖 Justificación: {candidato.get('Justificacion')}")
    print("==================================================")
